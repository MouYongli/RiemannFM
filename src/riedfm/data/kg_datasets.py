...
Supports two modes:
- "subgraph": Returns RieDFMGraphData objects (for pretraining / graph generation)
...
class RieDFMKGDataset(Dataset):
    ...
    Args:
        ...
        mode: "subgraph" (returns RieDFMGraphData) or "triple" (returns dict).
    ...
    def __getitem__(self, idx: int) -> RieDFMGraphData | dict:
        """Return a sample based on mode.

        mode="triple": Returns dict with head/relation/tail IDs.
        mode="subgraph": Returns RieDFMGraphData with sampled subgraph.
        """
    ...
    def _build_subgraph(self, h: int, r: int, t: int) -> RieDFMGraphData:
        ...
        return RieDFMGraphData(x=x, edge_types=edge_types, num_nodes=N, depth=depth)
