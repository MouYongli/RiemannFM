...
These datasets are used to evaluate RiemannFM's ability to transfer
...
class RiemannFMMolecularDataset(Dataset):
    """Molecular graph dataset wrapper.

    Wraps PyG molecular datasets (ZINC, QM9) into our RiemannFMGraphData format.
    ...
    def __getitem__(self, idx: int) -> RiemannFMGraphData:
        """Convert a PyG molecular graph to RiemannFMGraphData."""
        self._load_pyg_dataset()
        assert self._pyg_dataset is not None
