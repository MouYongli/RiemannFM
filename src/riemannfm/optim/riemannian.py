...
Four parameter groups:
  1. ``main``     : RieFormer backbone weights (2-D+ matrices), input/
                    time/encoding MLP weights, prediction-head weights,
  ...
  4. ``no_decay`` : Biases, LayerNorm weights, and any 1-D learnable
                    (e.g. ``mask_emb``). Same LR as ``main``, no WD.
                    Standard AdamW recipe — applying weight decay to
                    biases / 1-D normaliser weights drives sparsely-
                    updated 1-D params (e.g. ``mask_emb``, which only
                    receives gradient on the ~10% text-mask steps) to
                    zero exponentially.
...
# Defensive allowlist: parameters that must land in the no-decay group
# even if a future refactor changes their dimensionality.
_NO_DECAY_NAME_ALLOWLIST: frozenset[str] = frozenset({
    "pretrain_heads.mask_emb",
})
...
def _is_no_decay_param(name: str, param: torch.Tensor) -> bool:
    """Match parameters that should not receive weight decay.

    Matches biases, LayerNorm weights, and any other 1-D learnable
    (per the standard AdamW recipe used in BERT/GPT/LLaMA).
    """
    if name.endswith(".bias") or name in _NO_DECAY_NAME_ALLOWLIST:
        return True
    return param.ndim == 1
...
def build_optimizer(
    ...
    Returns:
        Configured optimizer with four param groups in the order
        ``[main, curvature, relation, no_decay]``.
    """
    ...
    for name, param in model.named_parameters():
        ...
        elif id(param) not in curvature_ids and _is_no_decay_param(name, param):
