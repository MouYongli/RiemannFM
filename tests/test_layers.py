...
class TestATHNorm:
    def test_output_shape(self):
        from riemannfm.models.normalization import RiemannFMATHNorm, RiemannFMTimestepEmbedding

        hidden_dim = 64
        time_dim = 32
        norm = RiemannFMATHNorm(hidden_dim, time_dim)
        t_embed_fn = RiemannFMTimestepEmbedding(time_dim)
...
class TestTimestepEmbedding:
    def test_output_shape(self):
        ...
        embed = RiemannFMTimestepEmbedding(64)
        t = torch.tensor(0.5)
        out = embed(t)
...
class TestGeodesicKernelAttention:
    def test_output_shape(self, manifold):
        ...
        attn = RiemannFMGeodesicAttention(hidden_dim=64, num_heads=4, use_mrope=False)
        h = torch.randn(8, 64)
...
class TestEdgeStreamAttention:
    def test_output_shape(self):
        ...
        attn = RiemannFMEdgeAttention(edge_dim=64, num_heads=4)
...
class TestDualStreamInteraction:
    def test_output_shapes(self):
        ...
        cross = RiemannFMDualStreamInteraction(node_dim=64, edge_dim=32)
...
class TestTextCrossAttention:
    def test_output_shape(self):
        ...
        cross = RiemannFMTextCrossAttention(node_dim=64, text_dim=128, num_heads=4)
        h_v = torch.randn(8, 64)
...
class TestVectorFieldHead:
    def test_continuous_head_shape(self, manifold):
        from riemannfm.models.heads import RiemannFMContinuousVFHead

        head = RiemannFMContinuousVFHead(hidden_dim=64, manifold=manifold)
        h = torch.randn(8, 64)
    ...
    def test_discrete_head_shape(self):
        from riemannfm.models.heads import RiemannFMDiscreteEdgeHead

        head = RiemannFMDiscreteEdgeHead(edge_dim=32, num_edge_types=11)
        h_e = torch.randn(8, 8, 32)
        logits = head(h_e)
