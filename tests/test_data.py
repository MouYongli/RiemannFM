...
class TestGraphData:
    def test_basic_creation(self, manifold):
        ...
        data = RiemannFMGraphData(x=x, edge_types=e, num_nodes=N)
...
class TestGraphCollator:
    def test_collation_shape(self, manifold):
        collator = RiemannFMGraphCollator(manifold, max_nodes=8)
        ...
        for n in [3, 5, 4]:
            x = manifold.sample_uniform((n,), DEVICE)
            e = torch.randint(0, 10, (n, n))
            graphs.append(RiemannFMGraphData(x=x, edge_types=e, num_nodes=n))
    ...
    def test_node_mask_correctness(self, manifold):
        collator = RiemannFMGraphCollator(manifold, max_nodes=6)

        x = manifold.sample_uniform((3,), DEVICE)
        e = torch.randint(0, 5, (3, 3))
        graphs = [RiemannFMGraphData(x=x, edge_types=e, num_nodes=3)]
    ...
    def test_auto_max_nodes(self, manifold):
        ...
        collator = RiemannFMGraphCollator(manifold)
